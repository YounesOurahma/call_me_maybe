"""Function selection via constrained decoding
against a registered catalog."""
...
class Decoder:
    ...
    def __init__(
        ...
        """Store the function catalog and pre-encode everything
    ...
    def apply(self, generated: List[int], logits: List[float]) -> np.ndarray:
        """Mask every token that cannot continue ``generated`` as a prefix
        of at least one registered function name."""
    ...
    def _allowed_next_tokens(self, generated: List[int]) -> Set[int]:
        """Return every token id that keeps ``generated`` a valid,
        not-yet-complete prefix of at least one candidate function name."""
        depth = len(generated)
    ...
    def _match(self, generated: List[int]) -> Optional[FunctionDefinition]:
        """Return the function whose full (token-encoded)
        name exactly equals ``generated``, if any."""
    ...
    def _build_instructions(self) -> List[int]:
        """Encode the static part of the prompt:
        instructions plus the full catalog of available functions.

        Computed once and reused for every call to :py:meth:`select`.
        """
        lines = [
            "",
